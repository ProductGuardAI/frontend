...
import { Badge, Empty, ErrorBox, human, Score, Spinner } from "../components";
...
export function Review() {
  ...
  if (q.error || !q.data)
    return <ErrorBox message={q.error} retry={q.reload} />;
  const p = q.data;
  return (
    <>
      {notice && (
      ...
      <div className="tab-content">
        ...
        {tab === "Product data" && <ProductData p={p} reload={q.reload} />}{" "}
        {tab === "Documents" && <Documents p={p} />}{" "}
        {tab === "Requirements" && <Requirements p={p} />}{" "}
        {tab === "Claims" && <Claims p={p} />}{" "}
        {tab === "Compliance findings" && <Findings p={p} reload={q.reload} />}{" "}
...
function Overview({ p, onTab }: { p: Product; onTab: (s: string) => void }) {
  const missing =
...
function ProductData({ p, reload }: { p: Product; reload: () => void }) {
...
function Documents({ p }: { p: Product }) {
  return (
    <section className="card">
      <div className="card-head">
        <div>
...
function Requirements({ p }: { p: Product }) {
  return (
    <section className="card">
      <div className="card-head">
        <div>
...
function Findings({ p, reload }: { p: Product; reload: () => void }) {
...
function ActivityTab({ p }: { p: Product }) {
  return (
