...
export function Products() {
  const q = useLoad(() => api<Product[]>("/products"));
    ...
    <>
      <div className="page-head">
        <div>
          ...
          <p>Review and manage every supplier onboarding case.</p>
        </div>
      ...
      <section className="card">
        <div className="filters">
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value)}>
            ...
            {[...new Set(q.data.map((p) => p.status))].map((s) => (
            ...
            <tbody>
              {list.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>
                    <b className={p.riskScore > 50 ? "danger" : ""}>
                      {p.riskScore}
                    </b>
                  </td>
