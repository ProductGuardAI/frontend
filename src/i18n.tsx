...
const vi:Record<string,string>={
 ...
 'Recent activity':'Hoạt động gần đây','Latest automated and reviewer events':'Các sự kiện AI và người duyệt mới nhất','Ready For Approval':'Sẵn sàng phê duyệt','Supplier Clarification Required':'Cần nhà cung cấp bổ sung','Compliance Review Required':'Cần xem xét tuân thủ',
 'LIVE GUIDED DEMONSTRATION':'TRÌNH DIỄN TƯƠNG TÁC TRỰC TIẾP','See ProductGuard AI find, explain, and route product risks':'Xem ProductGuard AI phát hiện, giải thích và phân luồng rủi ro sản phẩm','Reset all scenarios':'Đặt lại tất cả tình huống',
 'Open a scenario':'Mở một tình huống','Watch AI investigate':'Xem AI kiểm tra','Review the fix':'Xem giải pháp','Make the decision':'Đưa ra quyết định','Human approval stays mandatory':'Luôn cần con người phê duyệt','Open approval workspace':'Mở không gian phê duyệt',
 'Decision support only':'Chỉ hỗ trợ quyết định','Human review required':'Cần người đánh giá'
};
...
export function LanguageProvider({children}:{children:ReactNode}){const[language,setState]=useState<Language>(()=>(localStorage.getItem('productguard-language') as Language)||'vi');const setLanguage=(v:Language)=>{localStorage.setItem('productguard-language',v);setState(v)};const value=useMemo(()=>({language,setLanguage,t:(text:string)=>language==='vi'?(vi[text]??text):text}),[language]);return <Context.Provider value={value}>{children}</Context.Provider>}
